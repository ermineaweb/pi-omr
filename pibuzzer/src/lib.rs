...
pub struct Buzzer {
    ...
    pub pin_led: OutputPin,
}

impl Buzzer {
    pub fn new(pin_buzzer: OutputPin, pin_led: OutputPin) -> Buzzer {
        Buzzer {
            pin_buzzer,
            pin_led,
        }
    ...
    pub fn play_note(&mut self, note: &Note, duration: f32) {
        ...
        sleep(Duration::from_millis((duration * 1000.0) as u64 - 20));
        self.pin_buzzer.set_pwm_frequency(50.0, 0.0).expect("error");
        sleep(Duration::from_millis(20));
    }

    pub fn play_metronome(&mut self, tempo: u16) {
        let sound = Sound::new(Note::new(Head::A, Octave::O4), NoteValue::Half);
        let note_duration = 60.0 / tempo as f32;

        loop {
            let freq = sound.note.freq();
            println!("metronome - {}hz - {}s", freq, note_duration);
            self.pin_buzzer.set_pwm_frequency(freq, 0.5).expect("error");
            sleep(Duration::from_millis(40));

            self.pin_buzzer.set_pwm_frequency(50.0, 0.0).expect("error");
            sleep(Duration::from_millis((note_duration * 1000.0) as u64 - 40));
        }
    }
}
